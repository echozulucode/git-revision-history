...
*(Note: MacOS and Linux builds are supported via `npm run tauri build` but pre-built binaries are not yet distributed).*
...
3.  **Add Context:** In the large text area, type any "Adhoc Notes."
    -   *Example:* "Focus on the new Login UI performance improvements. Ignore the backend refactor."
...
The binary will be located at `./target/release/context_cli`.
...
```bash
context_cli --start v1.0.0 --end v1.1.0 --output context.md
```
...
```bash
context_cli --start HEAD~10 --end HEAD --ollama-model llama3 --output release_notes.md
```
...
```bash
context_cli --start v1.0 --end v1.1 --system-prompt docs/ai/release-notes-prompt.md --output full_prompt.md
```
